...
export default function GerenciarUsuarios() {
  ...
  // useForm para controlar os dados do formulário
  const { data, setData, patch, post, processing, reset } = useForm({
    name: '',
    permission_type_id: '',
    space_id: '',
    agendas: [] as string[],
    turnos: [] as string[], // adiciona os turnos aqui também
  ...
  useEffect(() => {
    if (userBeingEdited) {
      setData({
        ...
        turnos: [], // inicia vazio, depois ajusta
      ...
      if (userBeingEdited.space_id) {
        ...
        // Pegando os turnos que estão nas agendas do usuário
                  ...
                  {users.length > 0 ? (
                    users.map((u) => (
                      <tr key={u.id} className="hover:bg-muted/50 border-t">
                        ...
                        <td className="p-3">
                          {u.permission_type_id === 2
                            ? 'Gestor'
                            : u.permission_type_id === 1
                            ? 'Institucional'
                            : 'Comum'}
                        </td>
                        <td className="p-3">
